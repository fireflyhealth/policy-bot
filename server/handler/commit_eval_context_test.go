...
func TestCommitEvalContextParseConfigSuppressesStatusForUnseenPolicy(t *testing.T) {
	ec := makeCommitEvalContext(false)

	evaluator, err := ec.ParseConfig(context.Background(), common.TriggerCommit)
	require.Error(t, err)
	assert.Nil(t, evaluator)
	assert.Nil(t, ec.Status)
}

func TestCommitEvalContextParseConfigPostsStatusForSeenPolicy(t *testing.T) {
	ec := makeCommitEvalContext(true)
...
func makeCommitEvalContext(seenPolicy bool) *CommitEvalContext {
	return &CommitEvalContext{
		Options: &PullEvaluationOptions{
			StatusCheckContext: "policy-bot",
		},
		PublicURL: "https://policy-bot.example.com",
		CommitContext: &pulltest.Context{
			OwnerValue:     "testorg",
			RepoValue:      "testrepo",
			HeadSHAValue:   "abc123",
			BranchBaseName: "main",
			BranchHeadName: "gh-readonly-queue/main/pr-1-abc",
		},
		SkipPostStatus: true,
